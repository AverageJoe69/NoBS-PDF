...
use crate::{
    ...
    resolution::{detect_document_resolution, PageRasterBudget},
...
pub enum ExportError {
    #[error("document size scale must be between 10 and 100 percent, got {0}")]
    InvalidScale(u8),
...
/// Export using the desktop's detected per-page raster budgets.
pub fn export_for_scale(
    input: &Path,
    output: &Path,
    options: &ExportOptions,
    scale_percent: u8,
    ...
    if !(10..=100).contains(&scale_percent) {
        return Err(ExportError::InvalidScale(scale_percent));
    }
    export_with_scale_policy(input, output, options, scale_percent, progress)
}
...
fn export_with_scale_policy(
    ...
    let budgets = resolution.scaled_budgets(scale_percent);
    let config = PlannerConfig {
        document_target: None,
        page_raster_budgets: Some(budgets),
        ..Default::default()
    };
    progress("planning");
    let plan = create_plan(&inspection, &config);
    ...
    for image_plan in &plan.images {
        ...
        let Some(target) = image_plan.target_screen_pixel_dimensions else {
            summary.skipped += 1;
            image_results.push(result(
                source,
                "skipped",
                None,
                None,
                Some("target page raster dimensions unavailable".into()),
            ));
            continue;
        };
        match approve_jpeg(&document, source, target) {
